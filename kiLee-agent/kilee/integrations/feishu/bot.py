...
def run_bot():
    ...
    # Build HTTP client (for sending messages)
    ...
    # Build event handler with proper SDK types
    # encrypt_key / verification_token are not used in WebSocket mode
    # but are required by the builder; pass dummy values
    event_handler = lark.EventDispatcherHandler.builder(
        encrypt_key="",
        verification_token="",
    ).register_p2_customized_event(
        "im.message.receive_v1",
        lambda evt: asyncio.ensure_future(
            bridge.handle_incoming(evt.event, client)
        ),
    ).register_p2_customized_event(
        "im.message.message_read_v1",
        lambda evt: None,  # no-op — suppress "processor not found" error
    ).build()
    ...
    # Initialize bridge before starting WS
...
class FeishuBridge:
    ...
    async def handle_incoming(self, event: dict, client):
        logger.info("Feishu event received: %s", event.get("type", "?") if isinstance(event, dict) else type(event).__name__)
        identity = incoming_identity(event)
        if not identity["chat_id"]:
            logger.warning("No chat_id in event, skipping")
            return
    ...
    async def _cmd_threads(self, client, chat_id: str):
        try:
            chats_data = []
            for cid, state in self.store._data.get("chats", {}).items():
                chats_data.append({
                    "chat_id": cid,
    ...
    async def _send_text(self, client, chat_id: str, text: str):
        """Send a text message via Feishu API (synchronous under the hood)."""
        import lark_oapi.api.im.v1 as im_v1
        try:
            logger.info("Sending to Feishu chat %s: %s...", chat_id, text[:80])
            body = im_v1.model.CreateMessageRequestBody.builder() \
                .receive_id(chat_id) \
                .msg_type("text") \
            ...
            request = im_v1.CreateMessageRequest.builder() \
                ...
                .request_body(body) \
                .build()
            # This is a synchronous HTTP call — run in thread
            loop = asyncio.get_running_loop()
            ...
            if not response.success():
                ...
                logger.info("Feishu send OK to %s", chat_id)
        except Exception as e:
